...
class ConnectionManager:
    ...
    async def connect(self, websocket: WebSocket, job_id: str):
        ...
        await log_manager.log(
            f"WebSocket: Client connected to job {job_id} (Total: {len(self.active_connections[job_id])})",
            "backend",
        )
...
async def progress_websocket(websocket: WebSocket, job_id: str):
    ...
    try:
        ...
        await log_manager.log(
            f"WebSocket: Client disconnected from job {job_id}", "backend"
        )
    except Exception as e:
        connection_manager.disconnect(websocket, job_id)
        await log_manager.log(f"WebSocket Error on job {job_id}: {e}", "backend")
